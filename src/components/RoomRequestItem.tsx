...
const RoomRequestItem: React.FC<RoomRequestItemProps> = ({ request, onAccept, onDecline }) => {
  const title = request.user?.username
        ...
        {request.status === 'pending' ? (
          <>
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1 rounded bg-green-600 text-white"
            >
              <Check className="w-4 h-4" /> Accept
            </button>
            <button
              onClick={() => onDecline(request.id)}
              className="inline-flex items-center gap-1 px-3 py-1 rounded border"
            >
